 ...
 * تم تحسينه ليوفر تجربة مستخدم أفضل وواقعية أكثر
 */
...
function showDdosAttackTool() {
    ...
    if (typeof membershipSystem !== 'undefined' && membershipSystem.isLoggedIn) {
        if (!membershipSystem.isLoggedIn()) {
            ...
            if (typeof showCustomNotification === 'function') {
                showCustomNotification('يجب تسجيل الدخول أولاً لاستخدام ميزة هجمات DDoS', 'warning', {
                    title: 'تنبيه أمني',
                    icon: 'fas fa-user-shield',
                    duration: 5000
                });
            } else if (typeof showMessage === 'function') {
                showMessage('يجب تسجيل الدخول أولاً لاستخدام ميزة هجمات DDoS', 'warning');
            } else {
            ...
            <div class="modal-content dark-modal">
                ...
                <div class="modal-body px-4 py-4">
                    <div class="ddos-container">
                        ...
                        <div class="ddos-stats-container mb-4">
                            <div class="row">
                                <div class="col-md-3 col-6">
                                    <div class="ddos-stat-card">
                                        <div class="ddos-stat-icon"><i class="fas fa-tachometer-alt"></i></div>
                                        <div class="ddos-stat-value" id="ddos-stat-speed">0</div>
                                        <div class="ddos-stat-label">حزمة/ثانية</div>
                                    </div>
                                </div>
                                <div class="col-md-3 col-6">
                                    <div class="ddos-stat-card">
                                        <div class="ddos-stat-icon"><i class="fas fa-server"></i></div>
                                        <div class="ddos-stat-value" id="ddos-stat-connections">0</div>
                                        <div class="ddos-stat-label">اتصال نشط</div>
                                    </div>
                                </div>
                                <div class="col-md-3 col-6">
                                    <div class="ddos-stat-card">
                                        <div class="ddos-stat-icon"><i class="fas fa-exchange-alt"></i></div>
                                        <div class="ddos-stat-value" id="ddos-stat-bandwidth">0</div>
                                        <div class="ddos-stat-label">ميجابايت/ثانية</div>
                                    </div>
                                </div>
                                <div class="col-md-3 col-6">
                                    <div class="ddos-stat-card">
                                        <div class="ddos-stat-icon"><i class="fas fa-clock"></i></div>
                                        <div class="ddos-stat-value" id="ddos-stat-time">00:00</div>
                                        <div class="ddos-stat-label">الوقت المنقضي</div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    startAttackBtn.addEventListener('click', function() {
        ...
        const threadsCount = parseInt(document.getElementById('threads-count').value);
        ...
        if (!targetUrl || !attackMethod) {
            if (typeof showCustomNotification === 'function') {
                showCustomNotification('يرجى ملء جميع الحقول المطلوبة', 'error', {
        ...
        // تحديث الإحصائيات
        updateStats(0, 0, 0, '00:00');

        // بدء تحديث الوقت المنقضي
        elapsedTimeInterval = setInterval(function() {
            const elapsedTime = getElapsedTime(attackStartTime);
            document.getElementById('ddos-stat-time').textContent = elapsedTime;
        }, 1000);
        ...
        statsUpdateInterval = setInterval(function() {
            // محاكاة تغير الإحصائيات بشكل واقعي
            const progressPercentage = progress / 100;
            const maxPacketsPerSecond = getMaxPacketsPerSecond(attackMethod);
            const maxConnections = threadsCount;
            const maxBandwidth = (packetSize * maxPacketsPerSecond) / (1024 * 1024); // تحويل إلى ميجابايت/ثانية

            // إضافة بعض العشوائية للإحصائيات
            const randomFactor = 0.8 + (Math.random() * 0.4); // بين 0.8 و 1.2

            // حساب القيم الحالية
            const currentPacketsPerSecond = Math.floor(maxPacketsPerSecond * progressPercentage * randomFactor);
            const currentConnections = Math.floor(maxConnections * progressPercentage * randomFactor);
            const currentBandwidth = (maxBandwidth * progressPercentage * randomFactor).toFixed(2);

            // تحديث الإحصائيات في الواجهة
            updateStats(currentPacketsPerSecond, currentConnections, currentBandwidth, getElapsedTime(attackStartTime));
        }, 1000);
    });
    ...
    stopAttackBtn.addEventListener('click', function() {
        if (isAttacking) {
            ...
            // عرض إشعار
            if (typeof showCustomNotification === 'function') {
                showCustomNotification('تم إيقاف الهجوم بنجاح', 'info', {
                    title: 'إيقاف الهجوم',
    ...
    function completeAttack() {
        ...
        const packetsPerSecond = parseInt(document.getElementById('ddos-stat-speed').textContent);
        const connections = parseInt(document.getElementById('ddos-stat-connections').textContent);
        const bandwidth = parseFloat(document.getElementById('ddos-stat-bandwidth').textContent);
        const elapsedTime = document.getElementById('ddos-stat-time').textContent;

        // حساب إجمالي الحزم المرسلة
        const totalPackets = Math.floor(packetsPerSecond * parseTimeToSeconds(elapsedTime));

        addLogEntry(`[${getCurrentTime()}] اكتمل الهجوم`);
        addLogEntry(`[${getCurrentTime()}] إجمالي الحزم المرسلة: ${totalPackets.toLocaleString()}`);
        addLogEntry(`[${getCurrentTime()}] متوسط السرعة: ${packetsPerSecond.toLocaleString()} حزمة/ثانية`);
        addLogEntry(`[${getCurrentTime()}] متوسط عرض النطاق: ${bandwidth} ميجابايت/ثانية`);
        ...
        // عرض إشعار
        if (typeof showCustomNotification === 'function') {
            showCustomNotification(`اكتمل الهجوم بنجاح. تم إرسال ${totalPackets.toLocaleString()} حزمة.`, 'success', {
                title: 'اكتمال الهجوم',
    ...
    // تحديث الإحصائيات في الواجهة
    function updateStats(packetsPerSecond, connections, bandwidth, time) {
        document.getElementById('ddos-stat-speed').textContent = packetsPerSecond.toLocaleString();
        document.getElementById('ddos-stat-connections').textContent = connections.toLocaleString();
        document.getElementById('ddos-stat-bandwidth').textContent = bandwidth;
        document.getElementById('ddos-stat-time').textContent = time;
    }

    // الحصول على الوقت المنقضي بتنسيق MM:SS
    ...
    // تحويل الوقت من تنسيق MM:SS إلى ثواني
    function parseTimeToSeconds(timeString) {
        const [minutes, seconds] = timeString.split(':').map(Number);
        return (minutes * 60) + seconds;
    }

    // الحصول على الحد الأقصى لعدد الحزم في الثانية بناءً على نوع الهجوم
    function getMaxPacketsPerSecond(attackMethod) {
        const maxPackets = {
            'syn-flood': 15000,
            'http-flood': 8000,
            'udp-flood': 25000,
        ...
        return maxPackets[attackMethod] || 10000;
    }
