    ...
    async register(userData) {
        try {
            ...
            this.addAdminNotification(
                'طلب تسجيل جديد',
                `تم استلام طلب تسجيل جديد من ${userData.username} (${userData.fullName})`,
                'info'
            );

            // تحديث قائمة طلبات التسجيل في واجهة المستخدم إذا كان المستخدم الحالي مشرفًا
            if (this.isAdmin && typeof updatePendingRegistrationsList === 'function') {
                updatePendingRegistrationsList();
            }

            // إرسال طلب التسجيل إلى بوت تيليجرام إذا كان متاحًا
            if (typeof telegramBotSystem !== 'undefined') {
                telegramBotSystem.sendRegistrationRequest(registration)
                    .then(result => {
                        console.log('نتيجة إرسال طلب التسجيل إلى تيليجرام:', result);
                    })
                    .catch(error => {
            ...
            if (username === 'admin' && password === '@Black_00') {
                ...
                if (!adminUser) {
                    ...
                    adminUser = {
                        ...
                        /**
                         * الحصول على طلب تسجيل معلق بواسطة المعرف
                         * @param {string} id - معرف طلب التسجيل
                         * @returns {Object|null} - طلب التسجيل أو null إذا لم يتم العثور عليه
                         */
                        getPendingRegistrationById(id) {
                            return this.pendingRegistrations.find(reg => reg.id === id) || null;
                        }
                         ...
                         * @param {string} message - نص الإشعار
                         * @param {string} type - نوع الإشعار (success, info, warning, error)
                         * @returns {boolean} - هل تم إضافة الإشعار بنجاح
                         */
                        addAdminNotification(title, message, type = 'info') {
                            try {
                                // الحصول على إشعارات المشرفين الحالية
                                const adminNotificationsKey = 'greenhat_admin_notifications';
                                const adminNotificationsData = localStorage.getItem(adminNotificationsKey);
                                const adminNotifications = adminNotificationsData ? JSON.parse(adminNotificationsData) : [];
                                ...
                                console.log('تم إضافة إشعار للمشرفين:', notification);

                                // إظهار إشعار للمشرف الحالي إذا كان متصلاً
                                if (this.isAdmin && typeof showToast === 'function') {
                                    showToast(message, type);
                                }

                                return true;
                            } catch (error) {
                                console.error('خطأ في إضافة إشعار للمشرفين:', error);
                                return false;
                            }
                        }
                         ...
                         * @returns {Array} - قائمة إشعارات المشرفين
                         */
                        getAdminNotifications() {
                            try {
                                const adminNotificationsKey = 'greenhat_admin_notifications';
                                const adminNotificationsData = localStorage.getItem(adminNotificationsKey);
                                return adminNotificationsData ? JSON.parse(adminNotificationsData) : [];
                            } catch (error) {
                                console.error('خطأ في الحصول على إشعارات المشرفين:', error);
                                return [];
                            }
                        }
                        ...
                        markAdminNotificationAsRead(notificationId) {
                            try {
                                ...
                                // البحث عن الإشعار
                                const notificationIndex = adminNotifications.findIndex(notification => notification.id === notificationId);
                                if (notificationIndex === -1) {
                                    return false;
                                }

                                // تعليم الإشعار كمقروء
                                adminNotifications[notificationIndex].read = true;
                                ...
                                console.error('خطأ في تعليم إشعار المشرفين كمقروء:', error);
                                return false;
                            }
                        }
...
/**
 * تحديث قائمة طلبات التسجيل المعلقة (دالة مختصرة للتوافق)
 */
function updatePendingRegistrations() {
    console.log('استدعاء دالة updatePendingRegistrations');
    updatePendingRegistrationsList();
}
...
function updatePendingRegistrationsList() {
    console.log('تحديث قائمة طلبات التسجيل المعلقة');
    ...
    if (pendingRegistrationsList) {
        ...
        // إظهار عدد طلبات التسجيل المعلقة
        console.log(`عدد طلبات التسجيل المعلقة: ${pendingRegistrations.length}`);

        pendingRegistrations.forEach(registration => {
            const registrationItem = document.createElement('div');
            registrationItem.className = 'card mb-3 bg-dark border-secondary';
        ...
        // إظهار إشعار للمشرف إذا كان هناك طلبات تسجيل معلقة
        if (pendingRegistrations.length > 0 && membershipSystem.isAdminUser()) {
            showMessage(`لديك ${pendingRegistrations.length} طلب تسجيل معلق بانتظار الموافقة`, 'info');
        }
    } else {
        console.error('عنصر pending-registrations-list غير موجود في الصفحة');
    }
}
...
async function registerUser() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // تحديث قائمة طلبات التسجيل في لوحة المشرف إذا كان المستخدم مشرفًا
            if (membershipSystem.isAdminUser() && typeof updatePendingRegistrationsList === 'function') {
                updatePendingRegistrationsList();
            }

            // إذا كان هناك مشرف متصل، قم بإظهار إشعار له
            if (membershipSystem.isAdminUser()) {
                showMessage('تم استلام طلب تسجيل جديد من ' + username, 'info');
            }
        }
    } catch (error) {
        console.error('خطأ في التسجيل:', error);
...
async function approveRegistration(registrationId) {
    try {
        // الحصول على بيانات طلب التسجيل قبل الموافقة عليه
        ...
        if (approveButton) {
            ...
            if (result.success) {
                ...
                // إرسال إشعار للمستخدم (في الحالة الحقيقية سيتم إرسال بريد إلكتروني)
                console.log(`تم إرسال إشعار إلى ${registration.email} بالموافقة على طلب التسجيل`);

                // إضافة إشعار في النظام
                if (typeof addNotification === 'function') {
                    addNotification({
                        userId: registration.id,
                        message: 'تمت الموافقة على طلب التسجيل الخاص بك. يمكنك الآن تسجيل الدخول إلى حسابك.',
                        type: 'success',
